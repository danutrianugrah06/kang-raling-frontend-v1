    ...
    <div class="ip-table-card">
      ...
      <div class="ip-search-bar">
        <div class="ip-search-wrap">
          ...
          <input v-model="searchQuery" type="text" class="ip-search-input"
            placeholder="Cari desa, jenis sampah, atau pengelolaan..." aria-label="Cari data pengelolaan" />
        </div>
      </div>
      ...
      <div class="ip-table-wrap">
        <table class="ip-table" role="table" aria-label="Daftar data pengelolaan">
          ...
          <tbody>
            ...
            <!-- Data Rows -->
            <template v-else>
              <tr v-for="(item, index) in filteredData" :key="item.id || index" class="ip-data-row">
                <td class="ip-td-center">{{ (currentPage - 1) * perPage + index + 1 }}</td>
                <td>{{ formatTanggal(item.created_at) }}</td>
                <td>
                  <p class="ip-td-nama">{{ item.data_sampah && item.data_sampah.desa ? item.data_sampah.desa.nama_desa : '-' }}</p>
                  <p class="ip-td-sub"><i class="bi bi-box-seam"></i> {{ item.data_sampah && item.data_sampah.jenis_sampah ? item.data_sampah.jenis_sampah.nama : '-' }}</p>
                </td>
                <td>
                  <span class="ip-text-kelola">{{ item.jenis_pengelolaan ? item.jenis_pengelolaan.nama : '-' }}</span>
                </td>
                ...
                <td>
                  <div class="ip-aksi">
                    ...
                    <button class="ip-btn-icon ip-btn-hapus" @click.prevent.stop="konfirmasiHapus(item)" title="Hapus">
      ...
      <!-- Pagination -->
      <div v-if="!loading && dataList && dataList.length > 0" class="ip-table-footer">
        ...
        <div class="ip-pagination" role="navigation">
          ...
          <button v-for="pg in pageNumbers" :key="pg" :class="['ip-page-btn', { active: pg === currentPage }]" @click="goToPage(pg)">
            {{ pg }}
          </button>
    ...
    <div v-if="showModalForm" class="ip-modal-overlay" @click.self="tutupModalForm">
      <div class="ip-modal ip-modal-form">
        ...
        <div class="ip-modal-body">
          ...
          <!-- Dropdown Data Sampah Terverifikasi -->
          <div class="ip-form-group">
            ...
            <select v-else v-model="form.data_sampah_id" :class="['ip-form-select', { error: errors.data_sampah_id }]" :disabled="modeEdit">
              <option value="" disabled> Pilih Data Sampah </option>
              <option v-for="ds in listDataSampah" :key="ds.id" :value="ds.id">
                {{ ds.desa ? ds.desa.nama_desa : '-' }} - {{ ds.jenis_sampah ? ds.jenis_sampah.nama : '-' }} ({{ formatAngka(ds.jumlah) }} kg)
              </option>
            </select>
            <p v-if="errors.data_sampah_id" class="ip-form-error"><i class="bi bi-exclamation-circle"></i> {{ errors.data_sampah_id }}</p>
            <p v-if="!loadingDropdown && (!listDataSampah || listDataSampah.length === 0) && !modeEdit" class="ip-form-hint error">
              <i class="bi bi-info-circle"></i> Tidak ada antrean data sampah terverifikasi.
            </p>
          </div>

          <div class="ip-form-row">
            <!-- Dropdown Jenis Pengelolaan -->
            <div class="ip-form-group">
              ...
              <select v-else v-model="form.jenis_pengelolaan_id" :class="['ip-form-select', { error: errors.jenis_pengelolaan_id }]">
            ...
            <!-- Input Jumlah dengan validasi maksimal -->
            <div class="ip-form-group ip-form-group-jumlah">
              <label class="ip-form-label">
                Jumlah (Kg) <span>*</span>
              </label>
              <input v-model="form.jumlah" type="number" step="0.01" min="0" :max="maxJumlahPengelolaan" placeholder="0" :class="['ip-form-input', { error: errors.jumlah }]" />
              <p v-if="maxJumlahPengelolaan && !errors.jumlah" style="font-size: 12px; color: #16a34a; margin-top: 4px;">
                <i class="bi bi-info-circle"></i> Maksimal: {{ formatAngka(maxJumlahPengelolaan) }} Kg
              </p>
          ...
          <div class="ip-form-group">
            ...
            <textarea v-model="form.keterangan" class="ip-form-textarea" placeholder="Tuliskan keterangan jika ada..."></textarea>
          </div>
