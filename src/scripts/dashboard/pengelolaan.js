  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        this.showToast('Gagal memuat data jenis pengelolaan.', 'error')
      } finally {
        this.loading = false
      }
    },
    ...
    async simpanData() {
      ...
      if (!this.form.nama.trim()) {
        this.errors.nama = 'Nama jenis pengelolaan wajib diisi.'
        return
      }
      ...
      try {
        ...
        if (this.modeEdit) {
          await api.put(`/jenis-pengelolaan/${this.form.id}`, payload)
          this.showToast('Jenis pengelolaan berhasil diperbarui.', 'success')
        } else {
          await api.post('/jenis-pengelolaan', payload)
          this.showToast('Jenis pengelolaan berhasil ditambahkan.', 'success')
        }
    ...
    async hapusData() {
      ...
      try {
        await api.delete(`/jenis-pengelolaan/${this.modalHapus.id}`)
        this.showToast('Jenis pengelolaan berhasil dihapus.', 'success')
