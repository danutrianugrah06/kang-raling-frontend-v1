...
export default {
  ...
  computed: {
    ...
    labelPeriode() {
      const bulanLabel = this.filter.bulan ? (this.daftarBulan.find(b => b.value === this.filter.bulan)?.label || '') : 'Semua Bulan'
      const desaLabel = this.filter.desa_id ? (this.daftarDesa.find(d => d.id == this.filter.desa_id)?.nama_desa || '') : 'Seluruh Desa'
      
      // Mengubah '—' menjadi '-' (single dash) agar lebih rapi
  ...
  methods: {
    async fetchDesa() {
      try {
        ...
        this.daftarDesa = res.data.data || res.data
      } catch {}
    },
    ...
    async generateLaporan() {
      ...
      try {
        ...
        const [resSampah, resPengelolaan] = await Promise.all([
          api.get('/data-sampah', { params: { ...params, status: 'verified', per_page: 9999 } }),
          api.get('/data-pengelolaan', { params: { ...params, per_page: 9999 } }),
        ])

        const mentahSampah = resSampah.data.data || resSampah.data || []
        const mentahPengelolaan = resPengelolaan.data.data || resPengelolaan.data || []
    ...
    prosesDataSampah(dataMentah) {
      const groups = {}
      let tMasuk = 0, tOrg = 0, tAnorg = 0, tRes = 0
      
      const filterDesa = this.filter.desa_id
      const filterBulan = this.filter.bulan // Ambil data bulan dari filter
      ...
      dataMentah.forEach(row => {
        ...
        // Filter: Jika filter bulan diisi, tolak data yang bulannya beda (ambil digit ke 5-6 dari YYYY-MM-DD)
        ...
        if (!groups[key]) {
          groups[key] = {
            ...
            tanggal: tgl,
            organik: 0, anorganik: 0, residu: 0, total: 0,
            status: row.status
          }
        }
        ...
        if (jenis.includes('anorganik')) { groups[key].anorganik += qty; tAnorg += qty }
        else if (jenis.includes('organik')) { groups[key].organik += qty; tOrg += qty }
        else if (jenis.includes('residu')) { groups[key].residu += qty; tRes += qty }
      })

      this.tabelSampah = Object.values(groups).sort((a, b) => new Date(a.tanggal) - new Date(b.tanggal))
    ...
    prosesDataPengelolaan(dataMentah) {
      ...
      dataMentah.forEach(row => {
        const desaId = row.desa_id || row.data_sampah?.desa_id || '0'
        if (filterDesa && desaId != filterDesa) return

        const desaNama = row.desa?.nama_desa || row.data_sampah?.desa?.nama_desa || '-'
        const tgl = row.tanggal || row.created_at?.substring(0,10) || '-'
        
        // Filter bulan untuk data pengelolaan
        if (filterBulan && tgl !== '-' && tgl.substring(5, 7) !== filterBulan) return
      ...
      this.tabelPengelolaan = Object.values(groups).map(item => ({
        ...item,
    ...
    labelStatus(status) {
      const map = { pending: 'Pending', verified: 'Terverifikasi', rejected: 'Ditolak' }
